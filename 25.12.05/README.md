...
SLAM 目标：在 1～t 时间内，使用感知数据 $Z_{1:t} = \{Z_1,\ldots,Z_t\}$ 、收集的里程计数据 $U_{1:t} = \{U_1,\ldots,U_t\}$ 、初始起点 $X_0$ 的观测，来优化全局地图 $M$ 中记录的自定位位姿 $X_{1:t} = \{X_1,\ldots,X_t\}$ 和 n 个地标位置 $M = m_{1:n} = \{m_1,\ldots,m_n\}$ ,数学上概率公式被定义为：$$P(X_{1:t},M \mid Z_{1:t},U_{1:t},X_0)$$
